...
function getEntityBaseTypes(entityType: any, entityTypes: any) {
  ...
  while (entityType) {
    ...
    entityType = (entityTypes as Array<any>).find(entity => entity['$']['Name'] == baseTypeName);
    if (entityType) {
...
function parseEntitySets(namespace: string, entityContainer: any, entityTypes: any, annotations?: Array<Annotation>): Array<EntitySet> {
  return (entityContainer['EntitySet'] as Array<any>).map(entitySet => {
    const type = typeNameFromType(entitySet['$']['EntityType']);

    const entityType = (entityTypes as Array<any>).find(entity => entity['$']['Name'] == type);
...
function parseEntityType(entityType: any, entityTypes: Array<any>, namespace: string): EntityType {
  ...
  const entityBaseProperties = entityBaseTypes.flatMap(t => (t['Property'] || []).map(parseProperty));
    ...
    (navigationProperties as Array<any>).forEach(property => {
      ...
      if (type) { // OData V4 only
        ...
        if (type.startsWith('Collection(')) {
          ...
          const constraints = refConstraint ? (refConstraint as Array<any>).map(c => {
            return {
              property: c['$']['Property'],
...
function parseKey(key: any, properties: Array<EntityProperty>): Array<EntityProperty> {
  const refs = (key['PropertyRef'] as Array<any>).map(propertyRef => propertyRef['$']['Name'])
...
function parseActionAndFunctionParameters(parameters: Array<any>): Array<ActionAndFunctionParameter> {
...
function parseComplexTypes(complexTypes: Array<any>, schemas: Array<any>): Array<ComplexType> {
  return complexTypes && complexTypes.length ? complexTypes.map(t => {
    const schema = schemas.find(s => (s['ComplexType'] as Array<any>).find(ct => ct == t))
...
function parseEnumTypes(enumTypes: Array<any>, schemas: Array<any>): Array<EnumType> {
    return enumTypes && enumTypes.length ? enumTypes.map(t => {
        const schema = schemas.find(s => (s['EnumType'] as Array<any>).find(ct => ct == t))

        return {
            name: t['$']['Name'],
            memberNames: ((t['Member'] || []) as Array<any>).map(m => m['$']['Name']),
            namespace: schema ? schema['$']['Namespace'] : null
        }
    }) : [];
}
...
function parseAnnotations(annotations: Array<any>): Array<Annotation> {
  return annotations && annotations.length ? annotations.map(t => {
    return {
      target: t['$']['Target'],
      terms: ((t['Annotation'] || []) as Array<any>).map(a => a['$']['Term'])
    }
  }) : [];
}
...
function parseEntityTypes(entityTypes: Array<any>, schemas: Array<any>): Array<EntityType> {
  return entityTypes.map(et => {
    const schema = schemas.find(s => (s['EntityType'] as Array<any>).find(t => t == et))

    return parseEntityType(et, entityTypes, schema ? schema['$']['Namespace'] : '')
  });
}
...
function parse(xml: string): Promise<Service> {
  return new Promise<Service>((resolve, reject) => {
    ...
    const metadata = <xml2js.ElementCompact>xml2js.xml2js(xml, { compact: true, trim: true, alwaysArray: true, attributesKey: '$' });
